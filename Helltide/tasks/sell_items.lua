...
local task = {
    ...
    should_execute = function()
        return (get_local_player():get_item_count() >= 30 and settings.loot_mode == 1)
    ...
    execute = function()
        ...
        if not settings.has_interacted then
            local jeweler = utils.get_jeweler()
            if jeweler then
                console.print("Setting target to Jeweler: " .. jeweler:get_skin_name())
                explorer:set_custom_target(jeweler)
                explorer:move_to_target()

                if utils.distance_to(jeweler) < 2 then
                ...
                explorer:set_custom_target(enums.npc_positions.jeweler_position)
                explorer:move_to_target()
            end
        else
